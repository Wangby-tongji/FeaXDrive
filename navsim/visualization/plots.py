...
def plot_bev_with_agent(scene: Scene, scene_traj: Scene,agent: AbstractAgent) -> Tuple[plt.Figure, plt.Axes]:
    ...
    fig, ax = plt.subplots(1, 1, figsize=BEV_PLOT_CONFIG["figure_size"])
    add_configured_bev_on_ax(ax, scene.map_api, scene.frames[frame_idx])
    add_trajectory_to_bev_ax(right_ax, human_trajectory, TRAJECTORY_CONFIG["human"])
    add_trajectory_to_bev_ax(right_ax, agent_trajectory, TRAJECTORY_CONFIG["agent"])
    configure_bev_ax(ax)
    configure_ax(ax)
...
def plot_bev_and_camera_with_agent(
    ...
    trajectory_config = {
        ...
        "arrow_head_length_px": 20.0,
        "arrow_head_width_px": 15.0,
        "arrow_line_overlap_px": 3.0,
        "arrow_tangent_lookback": 4,
    }
