...
def _inside_image_bounds(pt: npt.NDArray[np.float32], image_width: int, image_height: int) -> bool:
...
def _clip_line_to_image(
    ...
    """
    Cohen–Sutherland line clipping against image rectangle.
    Returns clipped endpoints or None if segment does not intersect image.
    """
    ...
    while True:
        if not (code0 | code1):
            return np.array([x0, y0], dtype=np.float32), np.array([x1, y1], dtype=np.float32)
        if code0 & code1:
            return None
...
def _merge_clipped_segments_to_polylines(
    segments: List[Tuple[npt.NDArray[np.float32], npt.NDArray[np.float32]]],
    atol: float = 1e-4,
) -> List[npt.NDArray[np.float32]]:
    """
    Merge consecutive clipped segments into visible polylines while preserving gaps.
    """
    polylines: List[npt.NDArray[np.float32]] = []
    current: List[npt.NDArray[np.float32]] = []
    ...
    for seg_start, seg_end in segments:
        ...
        if np.allclose(current[-1], seg_start, atol=atol):
...
def add_trajectory_to_camera_ax(ax: plt.Axes, camera: Camera, trajectory: Trajectory, config: Dict[str, Any]) -> plt.Axes:
    """
    Robustly project the ego-frame trajectory into the camera image.

    Visual logic:
    1) clip every trajectory segment against the full image rectangle;
    2) merge consecutive visible segments into polylines;
    3) for the last visible polyline, trim its endpoint so the thick trajectory line
       ends *inside* the arrow head rather than creating a visible rectangular cap;
    4) draw the arrow head as a triangle whose tip is exactly the trajectory endpoint.
    """
    if trajectory is None or getattr(trajectory, "poses", None) is None or len(trajectory.poses) == 0:
        return ax

    poses_xy = np.asarray(trajectory.poses[:, :2], dtype=np.float32)
    poses_z = np.zeros((poses_xy.shape[0], 1), dtype=np.float32)
    poses_xyz = np.concatenate([poses_xy, poses_z], axis=1)

    # prepend current ego origin so the first visible segment can start from the ego position
    poses_xyz = np.concatenate([np.zeros((1, 3), dtype=np.float32), poses_xyz], axis=0)

    # Project with depth filtering only; image-bound clipping is handled below.
    pc_in_cam, positive_depth_mask = _transform_pcs_to_images(
        poses_xyz.T,
        camera.sensor2lidar_rotation,
        camera.sensor2lidar_translation,
        camera.intrinsics,
    ...
    clipped_segments: List[Tuple[npt.NDArray[np.float32], npt.NDArray[np.float32]]] = []
    visible_points: List[npt.NDArray[np.float32]] = []

    for i in range(len(pc_in_cam) - 1):
        if not (positive_depth_mask[i] and positive_depth_mask[i + 1]):
            continue

        p0 = np.asarray(pc_in_cam[i], dtype=np.float32)
        p1 = np.asarray(pc_in_cam[i + 1], dtype=np.float32)

        clipped = _clip_line_to_image(p0, p1, image_width=image_width, image_height=image_height)
        if clipped is None:
            continue
        ...
        if _inside_image_bounds(seg_start, image_width, image_height):
            visible_points.append(seg_start)
        if _inside_image_bounds(seg_end, image_width, image_height):
            visible_points.append(seg_end)

    polylines = _merge_clipped_segments_to_polylines(clipped_segments)

    # If there is no drawable segment but there is a visible point, still mark it.
    if len(polylines) == 0 and len(visible_points) > 0:
        unique_points = np.asarray(visible_points, dtype=np.float32)
        ax.scatter(
            unique_points[:, 0],
            unique_points[:, 1],
            s=max(18.0, float(config.get("line_width", 2.0)) * 10.0),
            c=config["line_color"],
    ...
    drawn_polylines: List[npt.NDArray[np.float32]] = [np.asarray(p, dtype=np.float32).copy() for p in polylines if len(p) >= 2]
    if not drawn_polylines:
        return ax

    # Find the last visible polyline/endpoint for arrow drawing.
    arrow_polyline_idx = None
    tip = None
    unit_dir = None
    seg_len = 0.0

    tangent_lookback = int(config.get("arrow_tangent_lookback", 3))
    tangent_lookback = max(tangent_lookback, 1)

    for idx in range(len(drawn_polylines) - 1, -1, -1):
        polyline = drawn_polylines[idx]
        if len(polyline) < 2:
            continue

        cur_tip = polyline[-1].astype(np.float32)
        ref_idx = max(len(polyline) - 1 - tangent_lookback, 0)
        ref_point = polyline[ref_idx].astype(np.float32)
        direction = cur_tip - ref_point
        cur_seg_len = float(np.linalg.norm(direction))

        if cur_seg_len <= 1e-6:
            for k in range(len(polyline) - 2, -1, -1):
                ref_point = polyline[k].astype(np.float32)
                direction = cur_tip - ref_point
                cur_seg_len = float(np.linalg.norm(direction))
                if cur_seg_len > 1e-6:
                    break
        ...
        if cur_seg_len > 1e-6:
            arrow_polyline_idx = idx
            tip = cur_tip
            unit_dir = direction / cur_seg_len
            seg_len = cur_seg_len
            break

    arrow_head = None
    trimmed_polylines = [p.copy() for p in drawn_polylines]

    if arrow_polyline_idx is not None and tip is not None and unit_dir is not None:
        normal = np.array([-unit_dir[1], unit_dir[0]], dtype=np.float32)

        head_length_px = float(config.get("arrow_head_length_px", 18.0))
        head_width_px = float(config.get("arrow_head_width_px", 14.0))
        line_overlap_px = float(config.get("arrow_line_overlap_px", 2.0))

        # Keep the head fully drawable on the last visible segment while allowing a
        # small overlap so the trajectory line is visually tucked under the arrow head.
        head_length_px = min(head_length_px, max(seg_len * 0.95, 2.0))
        head_width_px = max(2.0, min(head_width_px, max(head_length_px * 1.25, 2.0)))
        line_overlap_px = max(0.0, min(line_overlap_px, max(head_length_px - 1.0, 0.0)))

        base_center = tip - unit_dir * head_length_px
        trim_endpoint = tip - unit_dir * max(head_length_px - line_overlap_px, 1.0)
        left = base_center + normal * (head_width_px * 0.5)
        right = base_center - normal * (head_width_px * 0.5)

        trimmed_last = trimmed_polylines[arrow_polyline_idx].copy()
        trimmed_last[-1] = trim_endpoint
        trimmed_polylines[arrow_polyline_idx] = trimmed_last

        arrow_head = patches.Polygon(
            [tip, left, right],
            closed=True,
            facecolor=config["arrow_color"],
            edgecolor=config["arrow_edge_color"],
    ...
    # Draw trimmed polylines. Use a butt cap so the line does not leave a square block
    # protruding beyond the start of the arrow head.
    for polyline in trimmed_polylines:
        if len(polyline) < 2:
            continue
        ax.plot(
            polyline[:, 0],
