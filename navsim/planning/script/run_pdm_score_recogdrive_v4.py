...
from navsim.evaluate.dynamics_metrics import compute_dynamics_metrics
...
def run_pdm_score(args: List[Dict[str, Union[List[str], DictConfig]]]) -> List[Dict[str, Any]]:
    ...
    for idx, (token) in enumerate(tokens_to_evaluate):
        ...
        try:
            ...
            # Optional: log ΔΠ curve from projection-based diffusion (if enabled in the agent).
            ...
            if hasattr(agent, "last_delta_pi_curve") and agent.last_delta_pi_curve is not None:
                # Store as a list (will appear as an object column in the output CSV).
                score_row["delta_pi_curve"] = agent.last_delta_pi_curve

            # ------------------------------------------------------------
            # Optional validation: dynamics/feasibility metrics (speed/acc/jerk)
            # Useful for quantifying the effect of projection-based diffusion.
            # ------------------------------------------------------------
            if hasattr(cfg, "dynamics_eval") and getattr(cfg.dynamics_eval, "enable", False):
                ...
                # prefix to avoid name collisions with PDM scores
            ...
            # If the agent exposes ΔΠ curve, log it for this token.
...
def main(cfg: DictConfig) -> None:
    ...
    if dist.get_rank() == 0:
        ...
        # 只对数值列求均值（delta_pi_curve 这种 list/object 列会被自动排除）
        ...
        # If ΔΠ curves were collected (object column), also export a mean curve file.
        if "delta_pi_curve" in pdm_score_df.columns:
            ...
            for _, row in pdm_score_df.iterrows():
                ...
                if isinstance(c, list) and len(c) > 0:
                    curves.append(c)
            if len(curves) > 0:
                # Pad to max length and compute mean over scenarios.
