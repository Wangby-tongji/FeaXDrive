...
def main(cfg: DictConfig) -> None:
    ...
    tb_logger = TensorBoardLogger(save_dir=str(cfg.output_dir), name="lightning_logs")
    loggers = [tb_logger]
    ...
    callbacks = [
        pl.callbacks.ModelCheckpoint(
            ...
            save_top_k=5,
            every_n_epochs=1,
            filename="epoch={epoch}-step={step}-val_loss{val/loss_epoch:.4f}",
    ...
    trainer = pl.Trainer(
        **cfg.trainer.params,
        logger=loggers if rank == 0 else False,   # 关键：传进去；非0 rank 关掉
        callbacks=callbacks,
    )
