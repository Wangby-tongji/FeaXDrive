...
SPLIT = "test"  # ["mini", "test", "trainval"] #TODO
FILTER = "navlt_imp_pure_test" # TODO
# flag
config_dir = "/path/to/ReCogDrive/navsim/planning/script/config/common/train_test_split/scene_filter"
with initialize_config_dir(config_dir=config_dir, version_base=None):
    cfg = hydra.compose(config_name=FILTER)
# hydra.initialize(config_path="/path/to/ReCogDrive/navsim/planning/script/config/common/train_test_split/scene_filter")
# cfg = hydra.compose(config_name=FILTER)
...
agent = ReCogDriveAgent(
    ...
    checkpoint_path='/path/to/ReCogDrive/navsim/exp/navlt_v4/training_akappa/2026.02.05.12.50.28/lightning_logs/version_0/checkpoints/best.ckpt', # TODO
...
metric_cache_loader = MetricCacheLoader(Path('/path/to/ReCogDrive/navsim/exp/navlt_v4/metric_cache'))
...
def save_perfect_samples(entries, agent, metric_cache_loader, simulator, scorer):
    output_dir = "traj_plot_vis_test_navsim_navlt" #TODO
...
# file_path = '/path/to/ReCogDrive/navsim/exp/navlt_v4/recogdrive_agent_eval_akappa/imp/2026.02.06.11.11.12.csv'  # TODO
...
all_tokens = scene_loader.tokens  # 从 scene_loader 获取所有 token
all_entries = create_entries_from_tokens(all_tokens)

local_entries = all_entries[rank::world_size]
save_perfect_samples(local_entries, agent, metric_cache_loader, simulator, scorer)
print(f"Rank {rank}: 已保存 {len(local_entries)} 个样本 (all tokens)")