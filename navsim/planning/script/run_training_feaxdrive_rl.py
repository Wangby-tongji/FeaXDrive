...
def main(cfg: DictConfig) -> None:
    ...
    # Save checkpoints every epoch even when validation is skipped.
    # FA-GRPO/RL often uses max_epochs=1 and check_val_every_n_epoch=2;
    # in that case "val/loss_epoch" is not produced, so a monitor-only
    # checkpoint callback may save no ckpt.
    checkpoint_callbacks = [
        pl.callbacks.ModelCheckpoint(
            ...
            every_n_epochs=1,
            save_last=True,
            filename="best-{epoch:03d}-{val_loss_epoch:.4f}",
    ...
    trainer = pl.Trainer(**cfg.trainer.params, callbacks=checkpoint_callbacks)
