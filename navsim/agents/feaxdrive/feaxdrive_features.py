...
class FeaXDriveFeatureBuilder(AbstractFeatureBuilder):
    def __init__(self,
    ...
    def compute_features(self, agent_input: AgentInput) -> Dict[str, torch.Tensor]:
        ...
        if not self.cache_hidden_state:
            ...
            # idx = next((i for i, v in enumerate(high_command_one_hot) if v == 1), None)
            # command_str = navigation_commands[idx] if (idx is not None and idx < len(navigation_commands)) else "unknown"
            ...
            with torch.inference_mode():
                self.backbone.eval()
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16):  # 或 bfloat16
                    outputs = self.backbone(
                        pixel_values_cat.to("cuda", non_blocking=True),
                        questions,
