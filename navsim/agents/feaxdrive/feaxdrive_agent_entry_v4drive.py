...
from .feaxdrive_features import FeaXDriveFeatureBuilder ,TrajectoryTargetBuilder
from .feaxdrive_backbone import FeaXDriveBackbone
# The standard planner is the default for eps / dt / dyn / drivedyn.
# FA-GRPO keeps a separate planner because its reward scorer intentionally differs.
from .feaxdrive_diffusion_planner import (
    FeaXDriveDiffusionPlanner as StandardFeaXDriveDiffusionPlanner,
    FeaXDriveDiffusionPlannerConfig as StandardFeaXDriveDiffusionPlannerConfig,
)

# Backward-compatible public aliases.
FeaXDriveDiffusionPlanner = StandardFeaXDriveDiffusionPlanner
FeaXDriveDiffusionPlannerConfig = StandardFeaXDriveDiffusionPlannerConfig


class FeaXDriveAgent(AbstractAgent):
    def __init__(
        self,
        ...
        # ---- FA-GRPO reward hyperparameters ----
        # Used only when grpo=True / planner_variant=fagrpo.
        grpo_progress_weight: float = 10.0,
        grpo_ttc_weight: float = 5.0,
        grpo_comfortable_weight: float = 10.0,
        grpo_gamma_denoising: float = 0.6,
        grpo_bc_coeff: float = 0.1,
        vlm_size: Optional[str] = 'small', 
        train_backbone: bool = False,
        # ---- Feasibility-training hyperparameters ----
        # NOTE:
        #   use_constraint_projection is deprecated in the current official
        #   FeaXDrive chain. These parameters are still needed by dyn training
        #   because lambda_dyn > 0 activates the projector inside the planner
        #   for the training-time curvature/lateral-acc regularization.
        proj_dt: float = 0.5,
        ...
        drivable_sdf_source: str = 'data_map',
        ...
        self.planner_variant = str(planner_variant or 'standard').lower()
        ...
        if not self.cache_hidden_state and not self.cache_mode:
            ...
            self.backbone = FeaXDriveBackbone(
        ...
        if self.planner_variant in {"fa", "fagrpo", "fa_grpo"}:
            # FA-GRPO uses an isolated planner because its reward scorer differs
            # from both IL/dyn training and final standard PDM evaluation.
            from .feaxdrive_diffusion_planner_fagrpo import (
                FeaXDriveFAGRPODiffusionPlanner as planner_cls,
                FeaXDriveFAGRPODiffusionPlannerConfig as planner_config_cls,
            )
        elif self.planner_variant in {"standard", "base", "dyn", "drivedyn", "dt", "eps"}:
            planner_cls = StandardFeaXDriveDiffusionPlanner
            planner_config_cls = StandardFeaXDriveDiffusionPlannerConfig
        else:
            raise ValueError(f"Unknown FeaXDrive planner_variant: {planner_variant!r}")

        if self.dit_type == "large":
            cfg = make_feaxdrive_config(self.dit_type, action_dim=3, action_horizon=8, grpo=self.grpo, input_embedding_dim=1536,sampling_method=sampling_method, pred_type=diffusion_pred_type, config_cls=planner_config_cls)
        elif self.dit_type == "small":
            cfg = make_feaxdrive_config(self.dit_type, action_dim=3, action_horizon=8, grpo=self.grpo, input_embedding_dim=384,sampling_method=sampling_method, pred_type=diffusion_pred_type, config_cls=planner_config_cls)
        else:
        ...
        # Feasibility-training hyperparameters.
        # Do not set/use cfg.use_constraint_projection here: it is deprecated
        # in the current official chain. Training-time feasibility is controlled
        # by lambda_dyn/lambda_proj/lambda_proj_supervise and the projector
        ...
        # Source for inference-time SDF construction. Official FeaXDrive uses data_map.
        ...
        if self.grpo:
            ...
            if hasattr(cfg.grpo_cfg, "scorer_config"):
                cfg.grpo_cfg.scorer_config.progress_weight = float(grpo_progress_weight)
                cfg.grpo_cfg.scorer_config.ttc_weight = float(grpo_ttc_weight)
                cfg.grpo_cfg.scorer_config.comfortable_weight = float(grpo_comfortable_weight)
            if hasattr(cfg.grpo_cfg, "gamma_denoising"):
                cfg.grpo_cfg.gamma_denoising = float(grpo_gamma_denoising)
            if hasattr(cfg.grpo_cfg, "bc_coeff"):
                cfg.grpo_cfg.bc_coeff = float(grpo_bc_coeff)
            
        self.action_head = planner_cls(cfg).cuda()
    ...
    def get_feature_builders(self) -> List[AbstractFeatureBuilder]:
        return [FeaXDriveFeatureBuilder(
...
def make_feaxdrive_config(
    ...
    *,
    ...
    config_cls=StandardFeaXDriveDiffusionPlannerConfig,
) -> FeaXDriveDiffusionPlannerConfig:
    """
    A factory function to create a FeaXDriveDiffusionPlannerConfig object.
    ...
    Returns:
        FeaXDriveDiffusionPlannerConfig: An instantiated and configured planner config object.
    """
    size = size.lower()
    if size == "small":
