...
from .trajectory_projector import ProjectionLimits, ProxConfig, TrajectoryProjector
...
class ReCogDriveDiffusionPlanner(nn.Module):
    ...
    def _apply_drivable_guidance(self, x0_norm: torch.Tensor, index: torch.Tensor) -> torch.Tensor:
        ...
        try:
            ...
            self._timing_guidance_total_ms = float(getattr(self, "_timing_guidance_total_ms", 0.0)) + _toc_ms(t0, x0_norm.device)
            self._timing_guidance_calls = int(getattr(self, "_timing_guidance_calls", 0)) + 1
    ...
    def _project_x0_norm(self, x0_norm: torch.Tensor) -> torch.Tensor:
        ...
        try:
            ...
            self._timing_projection_total_ms = float(getattr(self, "_timing_projection_total_ms", 0.0)) + _toc_ms(t0, x0_norm.device)
            self._timing_projection_calls = int(getattr(self, "_timing_projection_calls", 0)) + 1
    ...
    def get_action(
        ...
        """
        timer_device = vl_features.device
        planner_t0 = _tic(timer_device)

        self._timing_guidance_total_ms = 0.0
        self._timing_guidance_calls = 0
        self._timing_projection_total_ms = 0.0
        self._timing_projection_calls = 0

        cond_t0 = _tic(timer_device)
        vl_embeds = self.feature_encoder(vl_features)
        ...
        condition_encode_ms = _toc_ms(cond_t0, timer_device)
        ...
        if self.config.sampling_method == 'flow':
            ...
            for step in range(self.config.num_inference_steps):
                ...
                step_ms_list.append(_toc_ms(step_t0, timer_device))
        ...
        planner_total_ms = _toc_ms(planner_t0, timer_device)
        sampling_steps = len(step_ms_list)
        sampling_total_ms = float(sum(step_ms_list))
        guidance_total_ms = float(getattr(self, "_timing_guidance_total_ms", 0.0))
        guidance_calls = int(getattr(self, "_timing_guidance_calls", 0))
        projection_total_ms = float(getattr(self, "_timing_projection_total_ms", 0.0))
        projection_calls = int(getattr(self, "_timing_projection_calls", 0))

        out: Dict[str, Any] = {
            "pred_traj": final_actions,
            "timing_planner_total_ms": float(planner_total_ms),
            "timing_condition_encode_ms": float(condition_encode_ms),
            "timing_sampling_total_ms": float(sampling_total_ms),
            "timing_sampling_steps": int(sampling_steps),
            "timing_sampling_step_mean_ms": float(sampling_total_ms / max(sampling_steps, 1)),
            "timing_guidance_total_ms": float(guidance_total_ms),
            "timing_guidance_calls": int(guidance_calls),
            "timing_guidance_mean_ms": float(guidance_total_ms / max(guidance_calls, 1)),
            "timing_projection_total_ms": float(projection_total_ms),
            "timing_projection_calls": int(projection_calls),
            "timing_projection_mean_ms": float(projection_total_ms / max(projection_calls, 1)),
        }
