...
from .recogdrive_diffusion_planner_entry_v4drivetime import (
...
class ReCogDriveAgent(AbstractAgent):
    def __init__(
        ...
        self.device = device
        self.last_timing_stats: Optional[Dict[str, float]] = None
    ...
    def forward(self, features: Dict[str, torch.Tensor], targets=None, tokens_list=None) -> Dict[str, torch.Tensor]:
        timer_device = torch.device(self.device) if torch.cuda.is_available() else None
        forward_t0 = _tic(timer_device) if not self.training else None
        backbone_ms = 0.0
        ...
        if self.training and not self.grpo:
            ...
            out = self.action_head.get_action(last_hidden_state.to(model_dtype), action_inputs)
            if isinstance(out, BatchFeature):
                out.data["timing_backbone_ms"] = float(backbone_ms)
                out.data["timing_forward_ms"] = float(_toc_ms(forward_t0, timer_device))
            return out
    ...
    def compute_trajectory(self, agent_input: AgentInput) -> Trajectory:
        ...
        total_t0 = _tic(timer_device)

        features: Dict[str, torch.Tensor] = {}
        feature_t0 = _tic(timer_device)
        for builder in self.get_feature_builders():
            features.update(builder.compute_features(agent_input))
        feature_build_ms = _toc_ms(feature_t0, timer_device)

        features = {k: v.unsqueeze(0) for k, v in features.items()}

        with torch.no_grad():
            predictions = self.forward(features)
            pred_dict = predictions.data if isinstance(predictions, BatchFeature) else predictions

            post_t0 = _tic(timer_device)
            poses = pred_dict["pred_traj"].float().cpu().squeeze(0)
            postprocess_ms = _toc_ms(post_t0, timer_device)

        total_ms = _toc_ms(total_t0, timer_device)

        self.last_timing_stats = {
            "timing_feature_build_ms": float(feature_build_ms),
            "timing_forward_ms": float(pred_dict.get("timing_forward_ms", 0.0)),
            "timing_backbone_ms": float(pred_dict.get("timing_backbone_ms", 0.0)),
            "timing_planner_total_ms": float(pred_dict.get("timing_planner_total_ms", 0.0)),
            "timing_condition_encode_ms": float(pred_dict.get("timing_condition_encode_ms", 0.0)),
            "timing_sampling_total_ms": float(pred_dict.get("timing_sampling_total_ms", 0.0)),
            "timing_sampling_steps": int(pred_dict.get("timing_sampling_steps", 0)),
            "timing_sampling_step_mean_ms": float(pred_dict.get("timing_sampling_step_mean_ms", 0.0)),
            "timing_guidance_total_ms": float(pred_dict.get("timing_guidance_total_ms", 0.0)),
            "timing_guidance_calls": int(pred_dict.get("timing_guidance_calls", 0)),
            "timing_guidance_mean_ms": float(pred_dict.get("timing_guidance_mean_ms", 0.0)),
            "timing_projection_total_ms": float(pred_dict.get("timing_projection_total_ms", 0.0)),
            "timing_projection_calls": int(pred_dict.get("timing_projection_calls", 0)),
            "timing_projection_mean_ms": float(pred_dict.get("timing_projection_mean_ms", 0.0)),
            "timing_postprocess_ms": float(postprocess_ms),
            "timing_agent_total_ms": float(total_ms),
        }
